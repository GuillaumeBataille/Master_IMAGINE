...
void Wave::modifData8(unsigned char *data)
...
void Wave::read(char *fileName)
{
    ...
    case 8:
        ...
        if (fread(data8, 1, data_nb, fd) != data_nb)
        { //Les donnees sont sur 8 bits
    ...
    case 16:
        ...
        if (fread(data16, 2, data_nb, fd) != data_nb)
        { //Les donnees sont sur 16 bits
            cout << "Wave::read: Erreur, impossible de lire dans le fichier " << fileName << " le bon nombre d'octet\n";
            exit(-1);
        }
        is_data8_allocated = false; // Tableau non alloué
...
void Wave::write(char *fileName)
{
    ...
    case 8:
        cout << "fwrite (data8, \n";
        if (fwrite(data8, 1, data_nb, fd) != data_nb)
        { //Les donnees sont sur 8 bits
        ...
        break;
    case 16:
        if (fwrite(data16, 2, data_nb, fd) != data_nb)
        { //Les donnees sont sur 16 bits
            cout << "Wave::write: Erreur, impossible d'écrire dans le fichier " << fileName << " le bon nombre d'octet\n";
            exit(-1);
        }
