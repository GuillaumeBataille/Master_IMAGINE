...
int display(const GMap3D &gmap)
{
    ...
    for (size_t i = 0; i < fdarts.size(); i++) // Pour chaque aretes
    {
        GMap::idlist_t orbit_i_darts = gmap.orbit({0, 1}, fdarts[i]);
    ...
    for (glm::vec3 id_n : indexed_normals)
    {
        std::cout << "id : " << id_n[0] << id_n[1] << id_n[2] << std::endl;
    }
    // for the property, you can first use a random value for each vertices.
    ...
    std::cout << " fdarts (nbr aretes) : " << fdarts.size() << std::endl;
    std::cout << " indices: " << indices.size() << std::endl;
    std::cout << " indexed_vertices: " << indexed_vertices.size() << std::endl;
    std::cout << " indexed_normals: " << indexed_normals.size() << indexed_normals[0][0] << " " << std::endl;
    std::cout << " property : " << property.size() << std::endl;
