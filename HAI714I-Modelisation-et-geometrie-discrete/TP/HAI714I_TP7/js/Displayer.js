class Displayer {
  ...
  static defaultModelSpace = {
    minX: 0,
    maxX: Displayer.width,
    minY: 0,
    maxY: Displayer.height,
  };
  currentTest;
  ...
  xm2p(x) {
    return (
      this.padding +
      ((this.cWidth - 2 * this.padding) * (x - this.minX)) /
        (this.maxX - this.minX)
    );
  }
  ...
  ym2p(y) {
    return (
      this.padding +
      ((this.cHeight - 2 * this.padding) * (y - this.maxY)) /
        (this.minY - this.maxY)
    );
  }
  ...
  intersect(p, mc) {
    ...
    let b = mc.x <= x + r && mc.x >= x - r && mc.y <= y + r && mc.y >= y - r;
  ...
  mDrawArrow(a, b) {
    let t = 0.2;
    let m1 = new Coord2D(a.x * t + (1 - t) * b.x, a.y * t + (1 - t) * b.y, "m");

    t = 0.9;
    let m2 = new Coord2D(a.x * t + (1 - t) * b.x, a.y * t + (1 - t) * b.y, "m");
    let c1 = m1.rotate(Math.PI / 20, b),
      c2 = m1.rotate(-Math.PI / 20, b);
    t = 0.1;
    let p = new Coord2D(a.x * t + (1 - t) * b.x, a.y * t + (1 - t) * b.y, "p");
  ...
  drawPointsComp(points, incrementalDrawing) {
    console.log("Call from drawPointsComp");
    this.setOptions(points, incrementalDrawing);
    let n = points.length,
      p;
    for (let i = 0; i < n; i++) {
