...
public:
    ...
    RaySceneIntersection computeIntersection(Ray const & ray) {
        ...
        for (unsigned int i = 0; i < spheres.size() ; i++ ){ // Pour toute les sphères rencontrées;
            raysphere = spheres[i].intersect(ray); // On teste l'intersection 
            
            if (raysphere.intersectionExists){ // Si l'intersection a bien lieu
                
                if(!result.intersectionExists) {// On regarde si on a déja eu une intersection, si c'est pas le cas 
            ...
            else{ // Si il avait déja rencontré un objet

                if (raysphere.t < result.t){ // On compare les t du nouveau et de l'ancien pour voir qui on garde, si le nouveau est plus petit que l'ancien
        ...
        for (unsigned int j = 0; j < squares.size() ; j++ ){
            ...
            if (raysquare.intersectionExists){ // Si l'intersection a bien lieu
                ...
                if (raysquare.t < result.t){ // On compare les t du nouveau et de l'ancien pour voir qui on garde, si le nouveau est plus petit que l'ancien
                //std::cout<<"on passe devant "<<std::endl;
                result.objectIndex = j; // On recup l'id de l'élément touché 
                result.typeOfIntersectedObject = 1; // On set a 1 le type d'objet rencontré (avec 0 pour sphère et 1 pour square )
                result.t = raysquare.t; // On recup son t, distance entre la caméra et le point d'intersection.
                result.raySquareIntersection = raysquare; // On récupère l'intersection
    ...
    Vec3 rayTrace( Ray const & rayStart ) {
      //  std::cout << "call a ray"<<std::endl;
        ...
        Vec3 color;
        RaySceneIntersection RSI = computeIntersection(rayStart);
       //      std::cout <<"Sphere t : "<< RSI.raySphereIntersection.t <<"   Square t :" <<RSI.raySquareIntersection.t<<std::endl;;
        if(RSI.intersectionExists){ // Si il y a bien une intersection avec un objet de la scène
            if(RSI.typeOfIntersectedObject == 0){ // On regarde si on est avec une sphère (0)
            color = spheres[RSI.objectIndex].material.diffuse_material; // Récup couleur
            }
            else if(RSI.typeOfIntersectedObject == 1){ // On regarde si on est avec une square (0)
