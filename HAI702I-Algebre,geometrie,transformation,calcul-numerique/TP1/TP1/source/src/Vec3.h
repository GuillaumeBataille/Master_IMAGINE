...
public:
    ...
    static float dot(Vec3 const &a, Vec3 const &b) {
        ...
        //TODO: Fonction à compléter // NEW 1.1
        float res = a.mVals[0]*b.mVals[0] + a.mVals[1]*b.mVals[1] + a.mVals[2]*b.mVals[2];; //Faire le calcul ici et assigner le resultat à res
        return res;
    }


    //Calculer le produit vectoriel entre 2 vecteurs // NEW 1.1
    ...
    Z = x1*y2 - y1*x2;
        ...
        Vec3 res(X,Y,Z); //Faire le calcul ici et assigner le resultat à res
      // std::cout<<" dans cross on a " << res[0] << res[1] << res[2]  << std::endl;
        return res;
    }
...
public:
    ...
    Vec3 operator*(const Vec3 &p) {
        //TODO: Fonction à completer //NEW 1.3 
        // Multiplier une mat3 a vec3 donne un nouveau vec3 transformé 
        float x = (*this)(0,0)*p[0] + (*this)(0,1)*p[1] + (*this)(0,2)*p[2];
        float y = (*this)(1,0)*p[0] + (*this)(1,1)*p[1] + (*this)(1,2)*p[2];
        float z = (*this)(2,0)*p[0] + (*this)(2,1)*p[1] + (*this)(2,2)*p[2];
    ...
    Mat3 operator*(const Mat3 &m2) { // calcul du produit matriciel m1.m2 //NEW 1.2
        ...
        float a_ = a*A+b*D+c*G;
        float b_ = a*B+b*E+c*H;
        float c_ = a*C+b*F+c*I;
        
        float d_ = d*A+e*D+f*G;
        float e_ = d*B+e*E+f*H;
        float f_ = d*C+e*F+f*I;

        float g_ = g*A+h*D+i*G;
        float h_ = g*B+h*E+i*H;
        float i_ = g*C+h*F+i*I;
        ...
        res = Mat3(a_,b_,c_,d_,e_,f_,g_,h_,i_);
        return res;
    }
