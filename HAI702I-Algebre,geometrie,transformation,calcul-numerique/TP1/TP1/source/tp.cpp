...
int main(int argc, char **argv) {
    ...
    Mat3 mat1(cos(A), -sin(A), 0, sin(A), cos(A), 0, 0, 0, 1);
    Mat3 mat2(cos(B), 0, sin(B), 0, 1, 0, -sin(B), 0, cos(B));
    Mat3 mat3(1,0,0,0, cos(C), -sin(C), 0, sin(C), cos(C));

    Mat3 mat_res = (mat1 * mat2) * mat3;

    float a = cos(A) * cos(B);
    float b = (cos(A) * sin(B) * sin(C)) - (sin(A) * cos(C));
    float c = (cos(A) * sin(B) * cos(C)) + (sin(A) * sin(C));

    float d = sin(A) * cos(B);
    float e = (sin(A) * sin(B) * sin(C)) + (cos(A) * cos(C));
    float f = (sin(A) * sin(B) * cos(C)) - (cos(A) * sin(C));

    float g = -sin(B);
    float h = cos(B) * sin(C);
    float i = cos(B) * cos(C);

    Mat3 rotation = Mat3(a,b,c,d,e,f,g,h,i);

    Mat3 scale =  Mat3 (1., 0., 0.,
                        0., 1., 0.,
                        0., 0., 1.);

    Mat3 transformation = rotation * scale;  
