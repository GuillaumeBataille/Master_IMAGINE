...
Vec3 project(Vec3 const &input_point, Plane const &i_plane) {
    ...
    //result= input_point _ Vec::cross((inp.))
    result = input_point-Vec3::dot((input_point-i_plane.point),i_plane.normal)*i_plane.normal;
    return result;
}
...
Vec3 project(Vec3 const &input_point, Vec3 const &i_origin, Vec3 const &i_axis) {
    ...
    result = i_origin+Vec3::dot((input_point-i_origin),i_axis)*(i_axis/i_axis.squareLength());
    return result;
}
...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        for (size_t j = 0; j< projection_on_basis.size(); j++){ // Pour toute projection 
        // variance[i] =...
        variance[i]+= pow((centroid[i]- projection_on_basis[j][i]),2); // On elève au carré la distance entre le centre et la proj
    }
    // Puis on normalise
    variance[i] /= projection_on_basis.size();
    } 
    ...
    Vec3 scale(1, 1, 3); //Mise à l'échelle non uniforme
    ...
    //NEW 1.1 : Modification de Mat3 transformation

   // transformation += Mat3(1,1,0, 0,,0, 0,0,-1);
    //Add a translation
