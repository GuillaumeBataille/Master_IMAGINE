...
public:
    ...
    Transform(Mat3 i_transformation = Mat3::Identity(), Vec3 i_translation = Vec3(0., 0., 0.))
        ...
        //attention, la plupars des transformations sont orthogonales, donc il n'y a pas besoin de calculer l'inverse
...
Vec3 project(Vec3 const &input_point, Vec3 const &i_origin, Vec3 const &i_axis) {
    ...
    Vec3 result = i_origin + Vec3::dot((input_point - i_origin), i_axis) * (i_axis/i_axis.squareLength());
    return result;
}
...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        for (unsigned int j = 0; j < projection_on_basis.size(); j++) {
            variance[i] += pow((centroid[i] - projection_on_basis[j][i]),2);
        }
