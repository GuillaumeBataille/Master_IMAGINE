...
char robot::front_trigger(char **Environnement)
{ // Retourne ce qu'on choppe en front de la position courante
    return Environnement[Front.r][Front.c];
}

char robot::left_trigger(char **Environnement)
{ // Retourne ce qu'on choppe en left de la position courante
    return Environnement[Left.r][Left.c];
}

char robot::back_trigger(char **Environnement)
{
    return Environnement[Back.r][Back.c];
}
...
char robot::long_right_trigger(char **Environnement)
{
    ...
    while (Environnement[add_in_FOV.r][add_in_FOV.c] == VIDE)
    {                                                                              // Tant qu'on est en train de regarder du vide (qu'on a pas rencontrer d'obstacle)
...
void robot::compute_direction()
{                                                                                 // Recompute les directions (après un rotate par exemple)
    ...
    robot::Back = add(current_pos, direction[mod(current_direction_id + 2, 4)]);
};
...
void robot::move(char **Environnement)
{
    ...
    else if (robot::front_trigger(Environnement) == 'M' || robot::front_trigger(Environnement) == 'P') // Cas ou on est face a un mur ou d'une porte
    {
        ...
        //Environnement[current_pos.r][current_pos.c] = 'r';
        compute_direction();
    }
...
void robot::next_move(char **Environnement)
{
    ...
    if (behavior == 1)
    {
        ...
        else // Sinon
        {
            ...
            if (Back == checkpoint) // Si lorsqu'on avance on retombe sur une case checkpoint, c'est qu'on a fait le tour de la salle sans retrouver de chaise, on a donc fini
            {
                checkpoint_clean = checkpoint;
            }
            if (current_pos == checkpoint_clean)
            {
                behavior = 4;
            }
        }
    ...
    {
        if (front_trigger(Environnement) != 'M')
        { // Tant qu'il n'est pas de retour collé au mur au checkpoint
            move(Environnement);
            compute_direction();
        }
        else
    ...
    else if (behavior == 4)
    {
        ...
        { // Tant qu'on a pas une porte a notre gauche
            move(Environnement);
        }
        else
        {
    ...
    {
        checkpoint = Position{0, 0};
        if (front_trigger(Environnement) == (VIDE))
        { // Tant qu'on a pas une porte en front
            move(Environnement);
        }
        else
        { // On est en face d'une porte
